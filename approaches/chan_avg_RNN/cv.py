...
import pandas as pd
import torch
from seq_rnn.dataset import (DatasetBuilder, SubjectMontageData,
    ...
    while not input_queue.empty():
        ...
        for i, (inner_train_valids, test_dataset) in enumerate(
            ...
            for j, (train_dataset, valid_dataset) in enumerate(
                ...
                # checkpoint named for the enumeration of learn / test splits
                elif args.nested_cross_val > 1:
                    checkpoint_suffix = str(i)
                # Multiple train / valid splits -
                # checkpoint named for the enumeration of train / valid splits
                ...
                model.load_state_dict(copy.deepcopy(initialized_parameters))
                trial_results = train(
                    subject, montage,
                    args, train_loader, valid_loader, test_loader, optimizer,
                    criterion, model, device, exp_dir,
                    checkpoint_suffix=checkpoint_suffix, verbose=args.verbose)
...
def train(subject: str, montage: str,
    ...
    for metric, value in metrics_valid.items():
        try:
            assert value == best_valid_metrics[metric]
