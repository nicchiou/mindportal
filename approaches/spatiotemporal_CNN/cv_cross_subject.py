...
from spatiotemporal_cnn.dataset import (CrossSubjectData, DatasetBuilder,
                                        LeaveOneOutDatasetBuilder)
...
def internal_model_runner(gpunum: int, args: argparse.Namespace, exp_dir: str,
    ...
    try:
        ...
        while not input_queue.empty():
            witholding_subject, montage_list = input_queue.get()
            ...
            if args.evaluation_method == 'trials':
                ...
                learn_data = CrossSubjectData(os.path.join(
                    ...
                    args.anchor, args.preprocessing_dir, args.data_path),
                    included_subjects=[],
                    excluded_subjects=[witholding_subject],
                    train_montages=montage_list,
                    classification_task=args.classification_task,
            ...
            for i, (inner_train_valids, test_dataset) in enumerate(
                ...
                for j, (train_dataset, valid_dataset) in enumerate(
                    ...
                    trial_results = train(
                        'all' if args.evaluation_method == 'trials'
                        else witholding_subject,
                        'all', args, train_loader, valid_loader, test_loader,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--evaluation_method', type=str,
                        choices=['trials', 'subjects'],
                        help='"trials" indicates evaluation of generalizaiton '
                        'to new trials (temporal) and "subjects" indicates '
                        'evaluation of generalization to unseen subjects.')
    ...
    exp_dir = os.path.join(
        ...
        f'{args.n_montages}_montages', 'cropped', 'cross_subject',
        args.expt_name)
    ...
    # Evaluate with train/valid/test split over all subject trials
    if args.evaluation_method == 'trials':
        input_queue.put((None, args.train_montages))
    # Evaluate on unseen subjects (val/test)
    elif args.evaluation_method == 'subjects':
        for subject in constants.SUBJECT_IDS:
            input_queue.put((subject, args.train_montages))
