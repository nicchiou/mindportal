...
class EncoderLayerForSAnD(nn.Module):
    """
    Encoder class that stacks individual encoder modules.
    """
    def __init__(
            self, input_features, seq_len, n_heads, n_layers, d_model=128,
        ...
        self.blocks = nn.ModuleList([
            modules.EncoderBlock(d_model, n_heads, dropout_rate)
...
class SAnD(nn.Module):
    ...
    <https://arxiv.org/abs/1711.03905>` Huan Song, Deepta Rajan, Jayaraman J.
    Thiagarajan, Andreas Spanias
    """
    def __init__(
            self, input_features: int, seq_len: int, n_heads: int, factor: int,
            n_layers: int, d_model: int = 128,
            dropout_rate: float = 0.2) -> None:
        super(SAnD, self).__init__()

        self.encoder = EncoderLayerForSAnD(input_features, seq_len, n_heads,
                                           n_layers, d_model, dropout_rate)
        self.dense_interpolation = modules.DenseInterpolation(seq_len, factor)
        self.clf = modules.ClassificationModule(d_model, factor)
